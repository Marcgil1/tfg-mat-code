...
s.t. Benefit_Definition
    {
        ...
        z[k,i] <= sum {l in 1..budgets[k]} prices[l] * v[l,i]
    ;
...
s.t. Zero_Benefit_If_No_Product_Acquired
    {
        ...
        z[k,i] <= prices[budgets[k]] * x[k,i]
    ;

s.t. Respect_Capacity_Constraints
    {
        i in ITEMS
    }:
        sum {k in CLIENTS} x[k,i] <= capacities[i]
    ;
